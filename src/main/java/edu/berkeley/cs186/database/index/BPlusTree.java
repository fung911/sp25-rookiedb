...
public class BPlusTree {
    ...
    public Optional<RecordId> get(DataBox key) {
        ...
        Optional<RecordId> recordId = leafNode.getKey(key);
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        // 1. call root.put(key, rid) recursively
        Optional<Pair<DataBox, Long>> result = root.put(key, rid);

        // 2. if root needs to split，create a brand new InnerNode as the root
        if (result.isPresent()) {
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        BPlusTreeIterator(LeafNode start, int index) {
            currentPage = start;
            this.index = index;
        }

        BPlusTreeIterator(BPlusNode start) {
            currentPage = start.getLeftmostLeaf();
            index = 0;
        }
        ...
        public boolean hasNext() {
            if (index < currentPage.getKeys().size()) return true;
            return currentPage.getRightSibling().isPresent();
        }
        ...
        public RecordId next() {
            ...
            if (index >= currentPage.getKeys().size()) {
                currentPage = currentPage.getRightSibling().get();
                index = 0;
            }
            return currentPage.getRids().get(index++);
        }
